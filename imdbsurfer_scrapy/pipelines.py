...
selectGenreByName = 'select id from imdbsurfer_webapp_genre where name = %s'
insertIntoGenre = 'INSERT INTO imdbsurfer_webapp_genre(dh_create, dh_update, name, user_create_id, user_update_id)' \
                  ' VALUES (now(), now(), %s, ({0}), ({0}))'.format(selectUserByEmail)

selectRoleByName = 'SELECT id FROM imdbsurfer_webapp_role where name = %s'
insertIntoRole = 'INSERT INTO imdbsurfer_webapp_role(dh_create, dh_update, name, user_create_id, user_update_id)' \
                 ' VALUES (now(), now(), %s, ({0}), ({0}))'.format(selectUserByEmail)

selectTypeByName = 'select id from imdbsurfer_webapp_type where name = %s'
insertIntoType = 'INSERT INTO imdbsurfer_webapp_type(dh_create, dh_update, name, user_create_id, user_update_id)' \
                 ' VALUES (now(), now(), %s, ({0}), ({0}))'.format(selectUserByEmail)

selectArtistByName = 'SELECT id FROM imdbsurfer_webapp_artist where name = %s'
insertIntoArtist = 'INSERT INTO imdbsurfer_webapp_artist(dh_create, dh_update, name, user_create_id, user_update_id)' \
                   ' VALUES (now(), now(), %s, ({0}), ({0}))'.format(selectUserByEmail)

selectArtistRole = 'SELECT id FROM imdbsurfer_webapp_artistrole' \
                   ' where artist_id = ({0})' \
                   ' and role_id = ({1})'.format(selectArtistByName, selectRoleByName)
insertIntoArtistRole = 'INSERT INTO imdbsurfer_webapp_artistrole(dh_create, dh_update, artist_id, role_id, user_create_id, user_update_id)' \
                       ' VALUES (now(), now(), ({0}), ({1}), ({2}), ({2}))'.format(selectArtistByName, selectRoleByName,
                                                                                   selectUserByEmail)

selectMovieByLink = 'SELECT id FROM imdbsurfer_webapp_movie where link = %s'
insertIntoMovie = 'INSERT INTO imdbsurfer_webapp_movie(name, year, rate, votes, link, metascore, minutes, watch, watched, dh_create, dh_update, user_create_id, user_update_id)' \
                  ' VALUES (%s, %s, %s, %s, %s, %s, %s, False, False, now(), now(), ({0}), ({0}))'.format(
    selectUserByEmail)
updateMovie = 'UPDATE imdbsurfer_webapp_movie SET dh_update=now(), rate=%s, votes=%s, metascore=%s, user_update_id=({0})' \
              ' WHERE link=%s'.format(selectUserByEmail)

selectMovieGenre = 'SELECT id FROM imdbsurfer_webapp_moviegenre' \
                   ' where genre_id = ({0})' \
                   ' and movie_id = ({1})' \
                   ' and type_id = ({2})'.format(selectGenreByName, selectMovieByLink, selectTypeByName)
insertIntoMovieGenre = 'INSERT INTO imdbsurfer_webapp_moviegenre(dh_create, dh_update, index, genre_id, movie_id, type_id, user_create_id, user_update_id)' \
                       ' VALUES (now(), now(), %s, ({0}), ({1}), ({2}), ({3}), ({3}))'.format(selectGenreByName,
                                                                                              selectMovieByLink,
                                                                                              selectTypeByName,
                                                                                              selectUserByEmail)
updateMovieGenre = 'UPDATE imdbsurfer_webapp_moviegenre SET dh_update=now(), index=%s, user_update_id=({0})' \
                   ' WHERE genre_id=({1})' \
                   ' and movie_id=({2})' \
                   ' and type_id = ({2})'.format(selectUserByEmail, selectGenreByName, selectMovieByLink,
                                                 selectTypeByName)

selectMovieArtistRole = 'SELECT id FROM imdbsurfer_webapp_movieartistrole' \
                        ' where "artistRole_id" = ({0})' \
                        ' and movie_id = ({1})'.format(selectArtistRole, selectMovieByLink)
insertIntoMovieArtistRole = 'INSERT INTO imdbsurfer_webapp_movieartistrole(dh_create, dh_update, "artistRole_id", movie_id, user_create_id, user_update_id)' \
                            ' VALUES (now(), now(), ({0}), ({1}), ({2}), ({2}))'.format(selectArtistRole,
                                                                                        selectMovieByLink,
                                                                                        selectUserByEmail)
...
class IndexUpdatePipeline(object):
    def __init__(self):
        self.connection = psycopg2.connect(get_connection())
        self.cursor = self.connection.cursor()
