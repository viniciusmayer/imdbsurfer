...
BEGIN
	...
	from imdbsurfer_webapp_movie m
		inner join imdbsurfer_webapp_moviegenre mg on mg.movie_id=m.id and m.id = _movie_id
		inner join imdbsurfer_webapp_genre g on g.id=mg.genre_id and g.id = _genre_id
		inner join imdbsurfer_webapp_type t on t.id=mg.type_id and t.id = _type_id;
